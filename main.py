...
# Inference
# Get ground truth
tgt_y_truth = torch.zeros(len(inference_data), 2)
for i, (src, tgt, tgt_y) in enumerate(inference_data):
    tgt_y_truth[i, :] = tgt_y

tgt_y_hat = torch.zeros((len(inference_data), 2), device=device)

# Perform inference
with torch.no_grad():
    for i, sample in enumerate(inference_data):
        src, tgt, tgt_y = sample
        # Maybe I have to do src.squeeze(0) to add a dimension of 1 at the beginning replacing the batch_size dimension. Probabli have to do it with tgt and maybe tgt_y too
        src, tgt, tgt_y = src.to(device), tgt.to(device), tgt_y.to(device)
