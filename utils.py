...
def generate_square_subsequent_mask(dim1: int, dim2: int) -> torch.Tensor:
    ...
    length (i.e. the length of the input sequence to the model),ç
...
def get_indices_input_target(num_obs, input_len, step_size, forecast_horizon, target_len):
    ...
    The indices will be used to split the data into sub-sequences on
    which models will be trained.
    ----------
    Arguments:
    num_obs (int): number of observations in the entire dataset for which indices
    must be generated.
    input_len (int): length of the input sequence (a sub-sequence of the entire
    data sequence)
    step_size (int): size of each step as the data sequence is traversed. If 1, the
    first sub-sequence will be indices 0-input_len, and the next will be 1-input_len.
    forecast_horizon (int): ow many index positions is the target away from the last
    index position of the input sequence? If forecast_horizon=1, and the input sequence
    ...
    input_len = round(input_len) # just as a precaution
    start_position = 0
    stop_position = num_obs - 1 # becuase of 0 indexing in Python
    ...
    indices = []
    while target_first_idx <= stop_position:
...
def get_indices_entire_sequence(data: pd.DataFrame, window_size: int, step_size: int) -> list:
    ...
    Returns a lsit of tuples. Each tuple is (start_idx, end_idx) of a subsequence. These tuples
    ...
    ----------
    Arguments:
    num_obs (int): number of observations (time steps) in the entire dataset for which indices
    must be generated, e.g. len(data).
    window_size (int): the desired length of each sub-sequence. Should be (input_sequence_length + 
    target_sequence_length). E.g. if you want the model to consider the past 100 time steps in order
    to predict the future 50 time_steps, window_size = 100 + 50 = 150.
